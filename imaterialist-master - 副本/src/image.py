# -*- coding: utf-8 -*-
"""
Created on Mon Apr 30 12:20:51 2018
...
class ImageDataGenerator(object):
    ...
    def flow_from_directory_with_labels(self, directory, y,
                            target_size=(256, 256), color_mode='rgb',
                            batch_size=32, shuffle=True, seed=None,
                            save_to_dir=None,
                            save_prefix='',
...
def _list_valid_filenames_in_directory_own(directory, white_list_formats, split,
                                        follow_links):
    """List paths of files in `subdir` with extensions in `white_list_formats`.
    # Arguments
        directory: absolute path to a directory containing the files to list.
...
class DirectoryIteratorOwn(Iterator):
    """Iterator capable of reading images from a directory on disk.
    # Arguments
        directory: Path to the directory to read images from.
            Each subdirectory in this directory will be
    ...
    def __init__(self, directory, y, image_data_generator,
                 target_size=(256, 256), color_mode='rgb',
                 batch_size=32, shuffle=True, seed=None,
                 data_format=None,
                 save_to_dir=None, save_prefix='', save_format='png',
        ...
        function_partial = partial(_count_valid_files_in_directory,
                                   ...
                                   follow_links=follow_links,
                                   split=split)
        self.samples = function_partial(directory)#sum(pool.map(function_partial
                                   #(directory)))
        ...
        results.append(_list_valid_filenames_in_directory_own(directory, white_list_formats, split, follow_links))
        for res in results:
            filenames = res           
            self.filenames += filenames
        ...
        #pool.close()
        #pool.join()
        super(DirectoryIteratorOwn, self).__init__(self.samples, batch_size, shuffle, seed)
    ...
    def _get_batches_of_transformed_samples(self, index_array):
        ...
        regular_list = []
        """
        x_list = batch_x.tolist()
        index_array_list = index_array.tolist()
        """
        for i, j in enumerate(index_array):
            fname = self.filenames[j]
            ### tolerate demaged pics
            try:
                img = load_img(os.path.join(self.directory, fname),
                               grayscale=grayscale,
                               target_size=self.target_size,
        ...
        for k in error_list:
            del x_list[k+1]
            del index_array_list[k+1]
        """
        # fixed the dimension bug, axis=0
        batch_x = batch_x.take(regular_list,axis=0)
        # take the correct labels
        index_array = index_array.take(regular_list,axis=0)
        #print("batch_x")
        #print(batch_x)
        #print("index_array")
       # print(index_array)
        ...
        if self.y is None:
            return output[0]
        output += (self.y[index_array],)
        print(batch_x.shape, self.y[index_array[0]], index_array)
        #print("output")
        #print(output)
        return output
