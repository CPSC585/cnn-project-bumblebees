...
from src.utils import exists, create_paths, check_opts, import_model, read_val_aug_params, read_train_labels_file
from src.utils import setup_paths, read_train_aug_params, read_unfreeze_layers, get_saved_models,read_val_labels_file
from src.iterators import get_std_iterator
...
def build_parser():
    ...
    parser.add_argument('--traintarget-path', type=str,
                        dest='train_target_path',
                        help='string path to the train label csv',
                        required=True)

    parser.add_argument('--valtarget-path', type=str,
                        dest='val_target_path',
                        help='string path to the val label csv',
                        required=True)
    ...
    parser.add_argument('--GPU-NUM', type=int,
                        dest='gpu',
                        help="numbers of GPU want to involve",
                        default=NUM_GPUS)
    return parser
...
if __name__ == '__main__':
    ...
    print(options.train_target_path)
    print(options.val_target_path)
    ...
    #get target label
    options = read_train_labels_file(options,logging)
    options = read_val_labels_file(options, logging)
    print(options.val_labels.shape, options.train_labels.shape)
    ...
    try:
        network = multi_gpu_model(network, gpus=options.gpu)
    ...
    # Create Generators
    train_generator = train_dataiter.flow_from_directory_with_labels(
        options.train_path, options.train_labels, seed=options.seed, color_mode='rgb',
        batch_size=options.batch_size, target_size=(448, 448), shuffle=False)# set false for testing shuffle=True real
    
    val_generator = val_dataiter.flow_from_directory_with_labels(
        options.val_path, options.val_labels, seed=options.seed,  color_mode='rgb',
    ...
    network.fit_generator(train_generator,
                          steps_per_epoch=int((1014545//options.batch_size)*0.8), #1014545//options.batch_size,#100//options.batch_size
                          epochs=options.epochs,
                          validation_data=train_generator,
                          validation_steps=int((1014545//options.batch_size)*0.2),#98975//options.batch_size,#10//options.batch_size
